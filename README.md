...
**Product site:** [relay.logvan.com](https://relay.logvan.com)
...
### Check-in System
- **PR badges** — pre-registered attendees get QR-coded badges printed in advance; scan and their Planning Center profile loads instantly
...
All configuration is via environment variables — nothing hardcoded. Copy `.env.example` to `.env` to get started.
...
| `EVENT_NAME` | ✅ | Event name shown in the app banner and written to PCO notes |
| `APP_PASSWORD` | ✅ | Shared password for staff login — rotate before each event |
...
| `PORT` | ⬜ | Server port — set automatically by Azure/Railway (default: 3000) |

> **Tip:** Rotate `PCO_APP_ID`/`PCO_SECRET` and `APP_PASSWORD` after every event.
...
- [ ] Migration from Azure App Service to Railway with GitHub auto-deploy (see [relay.logvan.com](https://relay.logvan.com))
