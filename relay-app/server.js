...
const EVENT_NAME = process.env.EVENT_NAME || 'Relay Event';
...
function trackSession(req) {
  ...
  if (!sid) return;
  const rawIp = req.headers['cf-connecting-ip'] || req.ip || 'unknown';
...
app.post('/auth/login', loginLimiter, (req, res) => {
  ...
  if (password === APP_PASSWORD) {
    ...
    const rawLoginIp = req.headers['cf-connecting-ip'] || req.ip || 'unknown';
    const ip = rawLoginIp.replace(/:\d+$/, '');
...
app.get('/api/config', (req, res) => {
  res.json({ locked_form_id: PCO_FORM_ID, event_name: EVENT_NAME });
});
...
app.post('/api/people/register', requireAuth, async (req, res) => {
  try {
    ...
    const creationNote = `${creatorTag}Walk-in registration — ${EVENT_NAME}`;
    await pcoPost(
      `https://api.planningcenteronline.com/people/v2/people/${personId}/notes`,
      {
        data: {
...
app.post('/api/offline/:badgeId/sync', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const syncNote = `${syncCreatorTag}Walk-in registration (offline capture) — ${EVENT_NAME}`;
...
app.get('/api/audit', requireAuth, async (req, res) => {
  try {
    ...
    // Enrich with person names from followups table
    const nameMap = {};
    if (followupsTable) {
      try {
        for await (const e of followupsTable.listEntities()) {
          nameMap[e.rowKey] = e.name || e.rowKey;
        }
      } catch(e) {}
    }
    logs.forEach(l => {
      if (l.entity_id && nameMap[l.entity_id]) {
        l.person_name = nameMap[l.entity_id];
      }
    });
...
app.listen(PORT, () => {
  console.log(`Conference scanner running at http://localhost:${PORT}`);
